        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <section className="bg-[#1a1a1a] rounded-3xl p-6 border border-slate-800/50 space-y-4">
            ...
            {(!formData.fotos || formData.fotos.length === 0) ? (
              <div 
                ...
                className={cn(
                  "flex items-center justify-center border-2 border-dashed border-slate-800 rounded-2xl p-8 text-slate-600",
                ...
                {formData.fotos.map((url, index) => (
                  <div key={index} className="relative aspect-square rounded-2xl overflow-hidden border border-slate-800 group">
                    {url.toLowerCase().endsWith('.mp4') || url.toLowerCase().endsWith('.mov') ? (
                      <video src={url} className="w-full h-full object-cover" controls={!isEditing} />
                    ) : (
                      <Image 
                        src={url} 
                        alt={`Foto ${index + 1}`} 
          ...
          {!isEditing && (
            <button 
              ...
              className="w-full flex items-center justify-center gap-3 px-8 py-4 bg-rose-500/10 text-rose-500 rounded-2xl text-xs font-black uppercase tracking-widest hover:bg-rose-500/20 transition-all"
            >

...
export default function EditBudgetPage() {
  ...
  const { items, addItem, removeItem, updateItem, updateItemComposition, updateCompositionItem, setItems, totals, variacaoAnual, setVariacaoAnual } = useOrcamento();
  ...
  const fetchProjects = async () => {
    const { data: projectsData } = await supabase.from('projetos').select('*').order('nome');
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setBudgetName(budget.nome);
        setBudgetDesc(budget.descricao || '');
        setSelectedProjectId(budget.projeto_id);
        setVariacaoAnual(budget.variacao_anual || 0);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // 2. Delete old items and insert new ones (simpler than syncing)
    ...
    <div className="flex-1 bg-[#0a0a0a] text-white overflow-y-auto custom-scrollbar p-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-black tracking-tight italic">Editar <span className="text-[#d4ff3f]">Orçamento</span></h1>
            <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest">Ajuste os itens e custos da obra</p>
          </div>
        </div>
          ...
          <div className="relative">
            <div className="bg-[#1a1a1a] border border-slate-800/50 rounded-2xl p-4 flex items-center gap-4 shadow-lg">
              ...
              <input 
                ref={searchInputRef}
                type="text" 
                placeholder="Buscar serviços TCPO..."
                value={query}
